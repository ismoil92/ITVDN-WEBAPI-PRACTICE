...
{
    #region FIELDS
    private string _filaPath;
    private static object _lock = new object();
    #endregion

    #region CONSTRUCTOR
    public FileLogger(string filePath) => this._filaPath = filePath;
    #endregion
    ...
    public void Log<TState>(LogLevel logLevel, EventId eventId, TState state, Exception? exception, Func<TState, Exception?, string> formatter)
    {
        lock (_lock)
        {
            if(exception != null)
            {
                ...
                string? message = $"Time:{time.ToShortTimeString()} Day:{time.ToShortDateString()}." +
                    $"Stack Trace:{exception?.StackTrace}\n\nMessage:{exception?.Message}\n====================================================\n\n";

                File.AppendAllText(_filaPath, formatter(state, exception!) + Environment.NewLine + message);
            }
        }
    }
    #endregion
